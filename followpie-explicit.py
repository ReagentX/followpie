...
#Enter the tag you want to like based on, keep the word in double quotes, do not put a # sign in front of the tag
TAGS = ["love"]
...
#Set the number of iterations you want to occur (eg, 100 times)
MAX_COUNT = 100

#MAX_SECS is the number of seconds to randomly wait between doing the next action
MAX_SECS = 1
...
def likePicture(pictureId):
    ...
    try:
        ...
        values = {'access_token' : auth_token,
                  'client_id' : client_id}
        valuess = {'access_token' : auth_token}
                  
        newLike = urlLike % (pictureId)
        print newLike
        data = urllib.urlencode(valuess)
